...
- Upload massal kartu produk (termasuk "Upload Produk Deal Lama via Excel" di halaman Registrasi Deal) memakai SATU importer: `uploadProductMasterList()` (export TAP → `products_tap`, upsert per (campaign_id, product_id)). Tidak ada importer `deal_products` lagi. Export TAP tidak membawa Tipe Campaign/Ads Budget/Service Fee: di jalur deal (`uploadDealProducts`) Tipe Campaign WAJIB dijawab sekali di form lalu diisikan ke semua kartu di file (Paid Campaign ikut mewajibkan kedua nominal, aturan dari `productCardIssues()` yang sama). Upload Master Product List di tab Produk TAP tidak menanyakannya → ketiga kolom tak disentuh.
