...
- TIDAK ADA field yang wajib per-kolom, `product_name` sekalipun (keputusan user 2026-08-14) — memaksa isian yang belum diketahui justru memancing data karangan. Satu-satunya kewajiban bersifat kondisional: Ads Budget & Service Fee wajib saat `campaign_type = paid` (Paid Campaign — satu-satunya tipe berbayar), dan HANYA di jalur PEMBUATAN kartu: form Registrasi Deal & upload deal, lewat `productCardIssues()` yang sama. Sesudah kartu ada, Ads Budget & Service Fee TIDAK lagi diedit dari tab Produk TAP maupun tab Deal Brand — keduanya dikelola per shop di tab Project BD (lihat di bawah), jadi form Edit Produk TAP & Edit shop Deal Brand tidak menyentuh kolom itu dan tidak menegakkan aturan Paid tersebut. Yang ditolak hanya submit yang kosong SELURUHNYA (`isEmptyProductCard()`): kartu tanpa satu pun isian tidak menyimpan informasi apa pun. Kartu tanpa `product_name` (seperti kartu tanpa Product ID) tersimpan dengan `needs_review=true`.
...
- Ads Budget & Service Fee TIDAK punya kolom di tabel Produk TAP (nominal per deal, bukan atribut export platform); fisiknya kolom per-kartu di `products_tap` yang DIJUMLAH per shop. Keduanya dibaca per shop di tabel "Shop dari Produk TAP" yang menjumlahkannya, bersama Deal by, PIC TAP, dan Nama BD (`uploaded_by_ids`, migrasi 0045; kolom Nama BD tunduk izin `products.view_owner_name`). SATU-SATUNYA tempat kedua nominal ini diedit dari tabel ringkasan = tombol Edit per shop di tab **Project BD** (tabel "Shop dalam Project"): form menerima satu nilai TOTAL per shop, dan `planShopBudgetEdit()` menaruh nilai penuh di satu kartu + menol-kan sisanya supaya penjumlahan view pas (tidak berlipat saat shop punya banyak kartu). Form Edit Produk TAP & Edit shop Deal Brand TIDAK menyentuhnya.
- Tab Deal Brand = SATU tabel: ringkasan kartu Produk TAP per Shop Name dari view `products_tap_shop_summary` (agregasi di SQL, bukan JS). (`brand_deals` deal lama masih tersimpan & disinkron untuk alert M4, tapi tabelnya tidak lagi ditampilkan di halaman ini.) Tabel shop punya tombol Edit yang menulis ke SELURUH kartu shop itu — Shop ID & Tipe Campaign saja (kolom lain berbeda per produk; Ads Budget/Service Fee dikelola di Project BD). Anggota grup dipilih lewat kolom generated `products_tap.shop_key` (0043), kunci yang sama dengan group by view — jangan menyalin ekspresinya ke filter aplikasi. Aturan Paid Campaign TIDAK ditegakkan di jalur ini: nominal tak lagi bisa diisi di sini, jadi menegakkannya berarti Tipe Campaign paid tak akan pernah bisa dipilih untuk shop yang kartunya belum berisi nominal.
...
- Tab **Project BD** (`/bd-projects`, migrasi 0044) = pengelompokan beberapa shop jadi satu campaign. Yang disimpan HANYA nama + daftar `shop_key`; jumlah kartu, ads budget, service fee, GMV, exp date DIBACA dari `products_tap_shop_summary` — jangan pernah menyalin angka ke `bd_projects`. Detail project punya tombol Edit per shop (tabel "Shop dalam Project") untuk **Ads Budget & Service Fee** shop itu: nilainya tetap ditulis ke `products_tap` (sumbernya, lewat `planShopBudgetEdit()` + `updateProjectShopBudget`), BUKAN ke `bd_projects` — aturan "angka tidak disimpan di project" tetap utuh. Tracking report campaign (report performance & Creator TC & Celeb) memakai tabel + parser yang SAMA dengan detail deal (`lib/deals/report-actions.ts`), bedanya kolom pemilik: tepat satu dari (`deal_id`, `project_id`) terisi. Template .xlsx kedua upload dibangun dari daftar kolom yang sama dengan yang dibaca parser (`lib/deals/report-template.ts`, dijaga test round-trip) — jangan menulis header template di tempat lain.
