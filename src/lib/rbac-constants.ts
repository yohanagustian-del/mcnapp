/**
 * Client-safe RBAC constants/types — no server-only imports (next/headers,
 * supabase/server, supabase/admin). Split out of rbac.ts so "use client"
 * components (e.g. tim/add-member-form.tsx) can import ROLES/Role/PERMISSIONS
 * without pulling the server-only auth helpers into the client bundle.
 * rbac.ts re-exports everything here, so existing `from "@/lib/rbac"` imports
 * are unaffected.
 */
