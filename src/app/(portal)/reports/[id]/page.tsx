...
export default async function ReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: report } = await supabase
    ...
    .select("id, creator_id, period_type, period_start, data_json, insight_draft, insight_final, status, token_used, generated_at")
    .eq("id", Number(id))
    .maybeSingle();
  if (!report) notFound();

  const d = report.data_json as ReportDataJson | null;
