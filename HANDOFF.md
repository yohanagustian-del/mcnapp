...
3. **Request CM massal (tab Kreator + /ingest)**: kartu "Kreator belum punya CM" kini punya centang untuk role CM (`creators.request_cm` tanpa izin assign) → tombol **Request N terpilih** (+ kolom alasan opsional) lewat action baru `requestCmAssignmentBulk` (maks 100/klik, skip yang sudah diajukan/sudah dipegang, 1 audit_logs per request, type `auto`; keputusannya tetap `approval` di `decideCmRequest`). Kolom "Request CM" menampilkan "Anda — menunggu ACC" / "N request menunggu" (penanda antrean untuk approver).
4. **Notifikasi approver**: menu **Kreator** di sidebar memakai badge jumlah request pending (`countPendingCmRequests`) — hanya untuk role dengan `creators.decide_cm_request` (Director/Head/SPV/CM Lead). Antrean terima/tolak tetap di panel "Request penugasan CM" di `/creators`.
5. **Special Project detail** (`/projects/[id]`): tabel **Metrik Harian** → komponen klien `daily-metrics-table.tsx` (sort semua header, paginasi **10/20/30**); tabel **Performa per Kreator** → `creator-performance-table.tsx` (sort semua header, paginasi **10/50/100**). Angka tetap dihitung di server (trackDaily + agregat `project_creator_metrics`) — komponen klien hanya urut/paginasi (CLAUDE.md #4). Konstanta ukuran halaman baru: `PAGE_SIZES_10_20_30`, `PAGE_SIZES_10_50_100`.
6. Verifikasi: typecheck 0 error, **695 pass + 2 skip** (naik dari 693: 2 tes baru — kelas Eksternal & sheet Petunjuk), `next build` sukses.
